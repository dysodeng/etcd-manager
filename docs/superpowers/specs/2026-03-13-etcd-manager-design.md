...
| POST | /api/v1/configs/:key/rollback | 回滚到指定版本，请求体 `{ "revision_id": 123 }` |
| POST | /api/v1/configs/import | 导入配置（JSON/YAML），key 已存在则覆盖，支持 dry-run 预览 |
...
- 支持 `Last-Event-ID` 头实现断线重连，服务端从指定 revision 开始重放
